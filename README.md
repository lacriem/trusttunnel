...
| `rules.toml` | Авто `client_random_prefix` + catch-all deny | Генерируется при создании пользователей; старые клиентские конфиги без префикса не подключатся |
...
### client_random_prefix и старые клиентские конфиги

Начиная с текущей версии скрипта, при создании или экспорте пользователя **автоматически** генерируется уникальный `client_random_prefix` и добавляется `allow`-правило в `rules.toml`. В конец `rules.toml` дописывается `catch-all deny`.

**Если у вас есть старые клиентские конфиги, созданные до этого обновления, они НЕ подключатся**, пока в них не будет добавлена строка:
```toml
client_random_prefix = "xxxx/xxxx"
```
(значение берётся из серверного `rules.toml` или путём повторного экспорта конфига через меню скрипта).

### Поведение пресетов

При применении любого пресета `vpn.toml` **полностью сбрасывается** к чистому `balanced` дефолту, а затем накладываются изменения выбранного пресета. Это исключает "наслоение" параметров при переключении между пресетами.
