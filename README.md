...
- **QUIC 0-RTT отключён** (`enable_early_data = false`) — защита от replay-атак
- **TCP idle timeout снижен** до 24 часов (`86400`) — предотвращает истощение ресурсов
- **HTTP/2 max frame size увеличен** до `65536` (RFC 7540 §6.5.2) — улучшает производительность
- **Доступ к приватным сетям сервера запрещён** (`allow_private_network_connections = false`)
- **`rules.toml`** содержит предупреждение о default-allow — рекомендуется настроить фильтрацию и завершить catch-all deny
